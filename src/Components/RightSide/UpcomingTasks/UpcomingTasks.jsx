...
export default function UpcomingTasks() {
  ...
  const [mostWorkingDay, setmostWorkingDay] = useState("");
 ...
 async function getUpcomingTasks(){
    setloading(true)
      let {data} = await axios.get('https://todo-list-be-tau.vercel.app/api/v1/user/tasks/upcoming' , {headers:{
        token:headerToken
      }})
    data.upcoming = data?.upcoming?.filter((t)=>t.finished === false) 
    data.upcoming = data?.upcoming?.sort(compare)
    if(data.upcoming.length) setpercentage(Math.ceil((data.upcoming.length/data.AlltasksNum)*100))
    else  setpercentage(0)
     const sorted = structuredClone(data.upcoming)
     let workCount = 1 
     let lastWorkCount = 1
    ...
    <div className="container">    
                <div className="row pt-5  p-res-5 ">
                    <div className="col-md-6  mb-2 ">
                        ...
                        Total Upcoming Tasks: 
                        {
                            loading?
                            <i class="ms-2 fa-solid fa-ellipsis fa-beat text-muted"></i>
                            :
                          <>
                                <span className="ms-2 lead text-muted">{Ttasks?.upcoming?.length}</span><br />
                        ...
                        Most Task-Packed Day: 
                        {
                            ...
                            <>
                                ...
                                <span className="fa-xs text-Grey">Task upcoming That Day <b className="ms-1 text-Grey-bold">{MostCount}</b></span>
                            </>
                        }
                        </div>
                    </div>
      ...
      <div className="p-res-3-rm  my-3 d-felx justify-content-center align-items-center">
        <div className="Container  ">
                <div className="w-100 bg-Blue rounded-top d-flex justify-content-center align-items-center text-white">
                    <div className="p-2"><i class="fa-solid fa-diagram-next text-white me-3 clickable text-white fa-2x"></i></div>
                </div>
                 ...
                 <div className="d-flex justify-content-center">
                    {
                        ...
                        <img src={require('../../../imgs/no Tasks.jpg')} className="w-50" alt="" title="No upcoming tasks"/>:""
                            ...
                            return<li class="list-group-item list-hover rounded-0 fadeIn " id={`${t._id}`}>
                              {
                                  ...
                                  :
                                  t.date != Ttasks.upcoming[i-1].date?
                                    <div className="d-flex justify-content-center align-items-center">
                                        <div className="bar-time rounded mb-2 me-2"></div>
                                        <div className="d-flex"><i class="fa-solid fa-calendar-day me-1" ></i><h6>{t.date.split('T')[0]}</h6></div>
                                        <div className="bar-time rounded mb-2 ms-2"></div>
